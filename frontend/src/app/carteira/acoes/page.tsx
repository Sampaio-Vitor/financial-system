...
export default function AcoesPage() {
  ...
  useEffect(() => {
    apiFetch<PositionsResponse>("/portfolio/ACAO")
      .then(setData)
      .catch(() => setData(null))
  ...
  if (loading) {
    return (
      <div>
        <h1 className="text-xl font-bold mb-6">Acoes (Brasil)</h1>
      ...
      <div>
        ...
        <div className="bg-[var(--color-bg-card)] rounded-xl border border-[var(--color-border)] p-8 text-center">
          <p className="text-[var(--color-text-muted)]">
            Nenhuma posicao em Acoes. Registre aportes para ver suas posicoes.
          </p>
        </div>
      </div>
    );
